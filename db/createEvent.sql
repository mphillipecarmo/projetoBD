...
CREATE EVENT atualiza_status_aluno
    ...
    DO
      BEGIN
        ...
        UPDATE reservas res inner join (SELECT usuario from reservas where status = 'Retirar' and timestampdiff(DAY, data , CURDATE()) > 2) atr on res.usuario = atr.usuario set status = 'Cancelada';
